...
//        if (elf_r_sym(r_info) == symidx && elf_r_type(r_info) == 22)
        if (elf_r_sym(r_info) == symidx && elf_r_type(r_info) == 1026)
        {
    ...
    for (uint32_t i = 0; i < reldyn_counts; i++)
    {
        ...
        if (elf_r_sym(r_info) == symidx &&
                (elf_r_type(r_info) == 257
                        || elf_r_type(r_info) == 1025))
        {
...
void elf_module::dump_rel_info(void)
{
    ...
    ElfW(Word) resszs[] = {(ElfW(Word))((unsigned long)this->m_reldyn_bytes/sizeof(ElfW(Rel))), (ElfW(Word))((unsigned long)this->m_relplt_bytes/sizeof(ElfW(Rel)))};
...
void elf_module::dump_rela_info(void)
{
    ...
    ElfW(Word) resszs[] = {(ElfW(Word))((unsigned long)this->m_reldyn_bytes/sizeof(ElfW(Rela))), (ElfW(Word))((unsigned long)this->m_relplt_bytes/sizeof(ElfW(Rela)))};
